...
/** 2/9/2010 Scott Atwell	@see AbstractControlUI.init(ControlT aControl, ParameterT aParameter, Atdl4jConfig aAtdl4jConfig) throws JAXBException
	...
	public DropDownListWidget(EditableDropDownListT control, ParameterT parameter) throws JAXBException {
		this.control = control;
		this.parameter = parameter;
		init();
	}
...
//TODO 1/24/2010 Scott Atwell added
		if ( ( dropDownList.isVisible() == false ) || ( dropDownList.isEnabled() == false ) )
		{
			return null;
		}
	...
	public String getParameterValue() {
		...
		if (control instanceof EditableDropDownListT && 
				...
				dropDownList.getText() != "")
		{
			// use the Parameter's Enum wireValue if the text matches a combo box item,
			// even if the dropdown was not used to select it
			for (int i = 0; i < dropDownList.getItems().length; i++)
			{
				if (dropDownList.getItems()[i].equals(dropDownList.getText()))
